...
const webDir = path.join(root, 'external', 'ekascribe-web');
...
function nativePackagePath(native) {
  ...
  return path.join(webDir, 'node_modules', ...pkgName.split('/'));
}

function missingNativeOptional(parentName) {
  const pkgJsonPath = path.join(webDir, 'node_modules', ...parentName.split('/'), 'package.json');
